...
class SeqRecDataset(Dataset):
    def __init__(self, interactions_path):
        ...
        with open(interactions_path, 'r', encoding='utf-8') as f:
            ...
            self.users = [rec["uid"] for rec in self.records]
    ...
    def __getitem__(self, idx):
        ...
        u_type = rec["u_type"] # int (0‒3)
        eval_from = rec["eval_from"] # int
        ...
        return {'user_id': user_id, 'sessions': user_sessions, 'u_type': u_type, 'eval_from': eval_from}
...
def seq_collate_fn(batch, use_delta_t=True, use_add_info=False):
    ...
    for user in batch:
        ...
        u_type_batch.append(user['u_type'])
        eval_from_batch.append(user['eval_from'])
    ...
    collate_dict['u_type']    = torch.tensor(u_type_batch,   dtype=torch.int8)
    collate_dict['eval_from'] = torch.tensor(eval_from_batch,dtype=torch.int32)
...
def get_dataloaders(**args):
    train_path = os.path.join(args["dataset_folder"], args["dataset_name"], 'timesplit', args["sampling_N"], "interactions_train_revised.jsonl")
    val_path = os.path.join(args["dataset_folder"], args["dataset_name"], 'timesplit', args["sampling_N"], "interactions_val_revised.jsonl")
    test_path = os.path.join(args["dataset_folder"], args["dataset_name"], 'timesplit', args["sampling_N"], "interactions_test_revised.jsonl")

    train_set = SeqRecDataset(train_path)
    val_set = SeqRecDataset(val_path)
    test_set = SeqRecDataset(test_path)
