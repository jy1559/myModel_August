...
class Sampler:
    ...
    def __init__(self, npz_path: str, device: torch.device | str = "cpu"):
        arr = np.load(npz_path)["values"]  # shape [n_items, 128]
        self.table = torch.as_tensor(arr, dtype=torch.long, device=device)
...
def compute_loss(
    ...
    *,
    ...
    elif cfg["model"] == "NARM" or cfg["model"] == "SR-GNN":
        return narm_loss(
            batch, model_out, model, cfg,
            candidate_emb=candidate_emb
        )
...
def narm_loss(
    ...
    * strategy/bin 개념 없이 "ALL" 한 묶음만 리턴.
    ...
    cache_npz = os.path.join(
        "/home/jy1559/Mar2025_Module/Datasets",
        cfg["dataset_name"],
        "precomputed_candidate.npz",
    )
    sampler = Sampler(cache_npz, device=device)
    cand_ids = sampler.sample(tgt_id, k=k_neg)      # [B', k]

    # ───────────────────── 2. 로짓 계산  ─────────────────────────────
    logits = _calc_logits(
        reps, cand_ids, model, candidate_emb,
        model_name=cfg["model"],  # type hinting
...
def tisasrec_loss(
    ...
    *,
    ...
    reps_sel = reps.view(-1, D)[sel_mask.view(-1)]          # [N,D]
    tgt_sel  = seq.view(-1)[tgt_mask.view(-1)]              # [N]
    ...
    logits  = _calc_logits(
        reps_sel, cand_id, model, "ID_64", 'TiSASRec',
        k_neg=k_neg
    ).float()
